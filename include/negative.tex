...
natural number. For infinitely branching types such as \icode{ℕTree}, however,
we cannot compute the height as a natural number, for to determine the height of
a \icode{node} we would have to take the maximum of infinitely many child tree
heights.

Thus, in the presence of infinitely branching types, sizes are better thought of
...
results are formalised in \icode{Ordinal.\allowbreak Shulman}.
...
\end{code}
The constructor is called \icode{limSuc} because it is the limit of the
successors of \icode{els}, an \icode{I}-indexed family of ordinals. \icode{Ord}
