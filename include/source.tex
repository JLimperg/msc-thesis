...
sizes as type-level natural numbers, \icode{Nat~n} is the type of naturals less
than or equal to \icode{n}. This intuition justifies the types of its
constructors:
\begin{code}
  zero : ∀ n < ↑ ∞. Nat n
...
equal to \icode{n} if and \icode{m~<~n}. If we were to define sized natural
...
definition of λST uses an eliminator instead.) The meaning of \icode{↑~∞} is
explained below.
...
size, is the upper bound of \icode{x} (meaning that \icode{x} can only be
instantiated with sizes below \icode{m}). At the term level, size variables are
introduced by binders of the form \icode{Λ~x~<~m} and terms with a
...
easily be added to Agda as well.\footnote{We need the zero size because we omit
  ...
  natural number successor that works on \icode{Nat~∞}, do not transfer directly
