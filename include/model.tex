...
A (semantic) \Def{size} is either a natural number or $∞$ or $⋆$. We define a
...
semantic successor. We thus define the \Def{successor} of a size $n$ as
\begin{displaymath}
  \mssuc(n) ≔
    \begin{cases}
...
\figref{typing:order}. Our size successor also preserves the relations $<$ and
$≤$.
...
\begin{AlignAnnot*}
  ⟦\sapp{t}{m}⟧_δ &∶& ⟦Γ⟧(δ) → ⟦\sub{T}{\Fill(m)}⟧ \\
    &=& ⟦Γ⟧(δ) → ⟦T⟧(δ, ⟦m⟧(δ)) &\quad \text{(by Lemmas \ref{lem:⟦subT⟧},
      \ref{lem:sub-app})} \\
